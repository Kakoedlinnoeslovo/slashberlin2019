...
class SegmentationStyleTransfer:
    def __init__(self, paths_config):
        ...
        #todo delete 'scream'
        for style in ('udnie', 'la_muse', 'wave', 'rain_princess', 'wreck'):
            self._style_transformers[style] = FSTWrapper(
                os.path.join(self._cloths_style_transfer_weights_dir, '{}.ckpt'.format(style)))

        self._style_transformers["jeans"] = self._style_transformers['wreck']
        self._style_transformers["leopard"] = self._style_transformers['wave']
        self._style_transformers["crocodile"] = self._style_transformers['la_muse']
        self._style_transformers["scream"] = self._style_transformers['rain_princess']
    ...
    def transform(self, init_path, image, params) -> np.ndarray:
        if init_path in self._cached_segmentations.keys():
            ...
            with Timer('Cloths Segmentation: %f'):
                ...
                #segmentation_mask= segmentation_mask/np.float(np.max(segmentation_mask))
                #segmentation_mask = (segmentation_mask!=0).astype(np.int8)

                #segmentation_mask = np.expand_dims(segmentation_mask, 2)
                #segmentation_mask = np.repeat(segmentation_mask, 3, 2)

                #image_gray = image[:,:,-1]
                #fg = image_gray * segmentation_mask
                #bg = image_gray*(1 - segmentation_mask)

                #plt.imshow(np.concatenate([fg, bg], axis=1), cmap="gray")
                #plt.show()
                #print("HERE")
